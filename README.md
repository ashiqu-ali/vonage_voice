...
The plugin's `AndroidManifest.xml` is merged automatically, so most permissions are already declared. You only need to complete two steps:

**1. Add the `google-services` plugin** to your app-level `build.gradle` (required for FCM incoming calls):
...
**2. Add `google-services.json`** — Download it from your Firebase project and place it at `android/app/google-services.json`. This enables FCM so incoming calls are delivered when the app is in the background or killed.
