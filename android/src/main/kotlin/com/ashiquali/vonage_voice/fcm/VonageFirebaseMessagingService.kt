package com.ashiquali.vonage_voice.fcm
...
import com.ashiquali.vonage_voice.service.VonageClientHolder
        ...
        if (clientWasNull) {
            ...
            client.setOnCallHangupListener { callId, quality, reason ->
                ...
                if (connection != null) {
                    ...
                    android.util.Log.i("VonageFCM",
                        "[FCM-HANGUP] pending invite found (answered elsewhere) — delegating to service for full teardown")
                    val cancelIntent = Intent(applicationContext, TVConnectionService::class.java).apply {
                        action = Constants.ACTION_CANCEL_CALL_INVITE
                        putExtra(Constants.EXTRA_CALL_ID, callId)
                    }
                    ...
                    // (race between local handleHangup and the SDK callback). Still broadcast
                    // CALL_ENDED so any alive Flutter engine / activity can dismiss the call
                    // screen — prevents a stuck UI after remote hangup.
                    android.util.Log.w("VonageFCM",
                        "[FCM-HANGUP] no connection/invite for callId=$callId — emitting fallback CALL_ENDED (race/double-cleanup)")
            ...
            // before the new session is ready, the existing invite-expiry fallback
            // in TVConnectionService covers the gap. Fire-and-forget — we never
            // block the FCM callback on this result.
            val revivalJwt = VonageClientHolder.getStoredJwt(applicationContext)
            if (revivalJwt != null) {
                ...
                client!!.createSession(revivalJwt) { error, sessionId ->
                    if (error != null) {
                        android.util.Log.w("VonageFCM", "[FCM-5] WebSocket revival failed: ${error.message} — invite-expiry fallback active")
                    } else {
                ...
                android.util.Log.w("VonageFCM", "[FCM-5] No stored JWT — cannot revive WebSocket. Invite-expiry fallback active")
            }
